...
const firebaseConfig = {
  ...
  authDomain: "takeprofit-tracker.firebaseapp.com",
  projectId: "takeprofit-tracker",
  storageBucket: "takeprofit-tracker.firebasestorage.app",
  messagingSenderId: "931306878330",
  appId: "1:931306878330:web:0f2f17aaca0b91743092c3"
};
...
async function loadRoster(){
  try{ const snap=await getDoc(doc(db,"config","roster")); return snap.exists()?snap.data().list:DEFAULT_STUDENTS; }
  catch(e){ console.error(e); return DEFAULT_STUDENTS; }
}
async function saveRoster(list){ try{ await setDoc(doc(db,"config","roster"),{list}); }catch(e){console.error(e);} }
async function loadStudentData(name){
  try{ const snap=await getDoc(doc(db,"students",name)); return snap.exists()?snap.data().data:emptyStudent(); }
  catch(e){ console.error(e); return emptyStudent(); }
}
    ...
    <tr style={{background:isLoss?"#1a0808":isWin?"#081a08":idx%2===0?"#12121f":"#0f0f1c",borderLeft:`3px solid ${isLoss?C.red:isWin?C.green:"#2a2a3e"}`}}>
      <td style={{...TD,width:32}}>
        <div style={{display:"flex",alignItems:"center",gap:3}}>
          ...
          {canRemove&&!locked&&(
            <button onClick={onRemove} style={{background:"none",border:"none",cursor:"pointer",color:"#ff444466",fontSize:15,padding:0,lineHeight:1}}
              onMouseEnter={e=>e.target.style.color=C.red} onMouseLeave={e=>e.target.style.color="#ff444466"}>×</button>
          )}
        </div>
      </td>
    ...
    <div style={{marginBottom:16,borderRadius:12,overflow:"hidden",border:`1px solid ${goalHit?C.green+"44":isLossDay?C.red+"44":"#1e1e2e"}`}}>
      ...
      {!locked&&(
        <div style={{padding:"8px 16px",background:"#0a0a14",borderTop:"1px solid #1e1e2e",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
          <button onClick={addTrade} style={{display:"flex",alignItems:"center",gap:7,background:"none",border:`1px dashed ${C.gold}55`,borderRadius:8,padding:"7px 16px",color:C.gold,fontSize:12,fontWeight:700,cursor:"pointer"}}
      ...
      {hasTrades&&(
        <div style={{padding:"6px 16px",background:"#0d0d18",borderTop:"1px solid #1e1e2e",display:"flex",gap:14,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {day.trades.map((t,i)=>{ const g=calcGross(t); if(g===null)return null; return <span key={i} style={{fontSize:11,fontFamily:"'DM Mono',monospace",fontWeight:700,color:g<0?C.red:C.green}}>T{i+1}: {fmt(g)}</span>; })}
      ...
      <div style={{background:"#0d0d14",borderTop:"1px solid #2d0d3d",padding:"10px 16px"}}>
        <div style={{fontSize:10,fontWeight:700,color:C.purple,letterSpacing:1,marginBottom:8}}>CREDIT SPREAD — BCS / BPS <span style={{fontWeight:400,color:"#444"}}>(optional)</span></div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end"}}>
          {[
            {lb:"TYPE",el:<select value={day.spread.type} disabled={locked} onChange={e=>updSpread("type",e.target.value)} style={{...SINP,border:"1px solid #4a0e6a"}}><option value="">— None —</option><option>BCS</option><option>BPS</option></select>},
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {sections.map((s,i)=>(
          <div key={i} style={{borderRadius:10,overflow:"hidden",border:`1px solid ${open===i?s.color+"55":"#1e1e2e"}`}}>
            <button onClick={()=>setOpen(open===i?null:i)} style={{width:"100%",background:open===i?`linear-gradient(135deg,${s.color}14,#12121f)`:"#12121f",border:"none",padding:"13px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",gap:10}}>
            ...
            {open===i&&(
              <div style={{background:"#0a0a12",borderTop:`1px solid ${s.color}33`,padding:"14px 20px"}}>
                {s.lines.map((line,li)=>(
                  <div key={li} style={{padding:"5px 0",fontSize:13,color:line?"#bbb":"transparent",borderBottom:li<s.lines.length-1?"1px solid #131320":"none",paddingLeft:line.startsWith(" ")?20:0}}>{line||"\u00a0"}</div>
                ))}
              </div>
            )}
          </div>
    ...
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:12}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          {["tracker","rules"].map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={{background:tab===t?C.gold:"#1a1a2e",color:tab===t?"#000":"#666",border:`1px solid ${tab===t?C.gold:"#2a2a3e"}`,borderRadius:8,padding:"8px 16px",fontWeight:700,fontSize:12,cursor:"pointer"}}>
              {t==="tracker"?"📊 Tracker":"📋 Rules & Guide"}
            </button>
          ))}
          <button onClick={handleSave} disabled={saving} style={{background:saved?C.green:C.gold,color:"#000",border:"none",borderRadius:8,padding:"10px 22px",fontWeight:800,fontSize:13,cursor:"pointer",opacity:saving?.7:1}}>
      ...
      {tab==="tracker"&&<>
        ...
        <div style={{display:"flex",gap:8,marginBottom:16}}>
          {WEEK_CONFIG.map((w,i)=>{
            ...
            return <button key={i} onClick={()=>setActiveWeek(i)} style={{flex:1,padding:"10px 8px",borderRadius:10,border:"none",cursor:"pointer",background:activeWeek===i?clr:"#12121f",color:activeWeek===i?"#fff":"#555",fontWeight:700,fontSize:12,borderBottom:activeWeek===i?`3px solid ${C.gold}`:"3px solid transparent"}}>
              <div>Week {w.week}</div>
    ...
    <div style={{background:"#12121f",borderRadius:12,border:"1px solid #D4A01733",marginBottom:24,overflow:"hidden"}}>
      ...
      <div style={{padding:"16px 20px"}}>
        <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:16,minHeight:36}}>
          {students.length===0&&<span style={{fontSize:12,color:"#444",fontStyle:"italic",alignSelf:"center"}}>No students enrolled yet.</span>}
          {students.map(name=>{
            const isConfirming=confirmRemove===name;
            return (
              <div key={name} style={{display:"flex",alignItems:"stretch",background:isConfirming?"#200808":"#1a1a2e",border:`1px solid ${isConfirming?C.red+"88":"#2a2a3e"}`,borderRadius:22,overflow:"hidden"}}>
                <span style={{padding:"6px 14px",fontSize:13,fontWeight:700,color:isConfirming?C.red:"#ddd",lineHeight:"20px"}}>{name}</span>
                <button onClick={()=>handleRemoveClick(name)}
                  style={{background:isConfirming?C.red+"22":"transparent",border:"none",borderLeft:`1px solid ${isConfirming?C.red+"66":"#2a2a3e"}`,padding:"6px 11px",cursor:"pointer",color:isConfirming?C.red:"#555",fontSize:isConfirming?10:16,fontWeight:isConfirming?800:400,lineHeight:"20px"}}
        ...
        {confirmRemove&&(
          <div style={{background:"#1a1000",border:"1px solid #D4A01744",borderRadius:8,padding:"8px 14px",marginBottom:14,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
            <span style={{fontSize:12,color:C.amber}}>⚠️ Click <strong>CONFIRM?</strong> again to remove <strong>{confirmRemove}</strong>. Their data stays saved.</span>
            <button onClick={()=>setConfirmRemove(null)} style={{background:"none",border:"1px solid #444",borderRadius:6,padding:"4px 10px",color:"#666",fontSize:11,cursor:"pointer"}}>Cancel</button>
          </div>
        )}
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          <input type="text" placeholder="New student name..." value={newName}
            onChange={e=>{ setNewName(e.target.value); setErr(""); }}
            onKeyDown={e=>e.key==="Enter"&&handleAdd()}
            style={{background:"#0d0d1a",border:`1px solid ${err?C.red:C.gold+"66"}`,borderRadius:8,padding:"9px 14px",color:"#e0e0e0",fontSize:13,fontFamily:"'DM Mono',monospace",outline:"none",width:220}}/>
          <button onClick={handleAdd} style={{background:C.gold,color:"#000",border:"none",borderRadius:8,padding:"9px 20px",fontWeight:800,fontSize:13,cursor:"pointer"}}>＋ Add Student</button>
        ...
        <div style={{padding:"14px 20px",borderBottom:"1px solid #1e1e2e",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div style={{display:"flex",gap:6}}>
            {[["total","By P&L"],["name","By Name"]].map(([s,l])=>(
              <button key={s} onClick={()=>setSortBy(s)} style={{background:sortBy===s?C.gold:"#1a1a2e",color:sortBy===s?"#000":"#666",border:"none",borderRadius:6,padding:"4px 12px",fontSize:11,fontWeight:700,cursor:"pointer"}}>{l}</button>
            ))}
          </div>
        </div>
        {rows.length===0
          ? <div style={{padding:"32px",textAlign:"center",color:"#444",fontSize:13,fontStyle:"italic"}}>No students enrolled yet.</div>
          : <div style={{overflowX:"auto"}}>
                  ...
                  {sorted.map((row,i)=>{
                    ...
                    return (
                      <tr key={row.name} style={{background:i%2===0?"#0f0f1c":"#12121f",borderLeft:`3px solid ${row.passed?C.green:"#1e1e2e"}`}}>
                        ...
                        {row.weeks.map((w,wi)=><td key={wi} style={{...TD,fontFamily:"'DM Mono',monospace",fontSize:12,color:w.passed?C.green:w.total<0?C.red:"#666"}}>{fmt(w.total)}{w.passed?" ✅":""}</td>)}
            ...
            : <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
                {students.map(s=>(
                  <button key={s} onClick={()=>{setSel(s);setPin("");setErr("");}}
                    style={{background:sel===s?C.gold:"#1a1a2e",color:sel===s?"#000":"#888",border:`1px solid ${sel===s?C.gold:"#2a2a3e"}`,borderRadius:10,padding:"12px 8px",fontWeight:700,fontSize:14,cursor:"pointer"}}>
                    {s}
                  </button>
                ))}
              </div>
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const unsubs=students.map(name=>
      onSnapshot(doc(db,"students",name),snap=>{
        if(snap.exists()) setAllData(prev=>({...prev,[name]:snap.data().data}));
      })
    );
  ...
  const login=async who=>{
    ...
    setStudents(roster);
    const res={};
    for(const s of roster){ res[s]=await loadStudentData(s); }
    setAllData(res);
    setUser(who);
    setLoading(false);
  };

  const handleSave=async(name,data)=>{ await saveStudentData(name,data); setAllData(prev=>({...prev,[name]:data})); };
  const handleAddStudent=async name=>{ const updated=[...students,name]; setStudents(updated); await saveRoster(updated); setAllData(prev=>({...prev,[name]:emptyStudent()})); };
  const handleRemoveStudent=async name=>{ const updated=students.filter(s=>s!==name); setStudents(updated); await saveRoster(updated); if(activeStudent===name){ setActiveStudent(null); setView("dashboard"); } setAllData(prev=>{ const n={...prev}; delete n[name]; return n; }); };

  if(!user) return <LoginScreen onLogin={login} students={students}/>;
  if(loading) return <div style={{minHeight:"100vh",background:C.dark,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{color:C.gold,fontSize:18,fontFamily:"'DM Mono',monospace"}}>Loading...</span></div>;
    ...
    <div style={{minHeight:"100vh",background:C.dark,color:"#e0e0e0",fontFamily:"'DM Sans',sans-serif"}}>
      <style>{`
        ...
        *{box-sizing:border-box} input,select{box-sizing:border-box}
        ...
        ::-webkit-scrollbar{width:6px;height:6px} ::-webkit-scrollbar-track{background:#0d0d1a} ::-webkit-scrollbar-thumb{background:#2a2a3e;border-radius:3px}
      `}</style>
      <div style={{background:"#0d0d1a",borderBottom:"1px solid #1e1e2e",padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",height:52,position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
          {isCoach?(
            <>
              <button onClick={()=>{setView("dashboard");setActiveStudent(null);}} style={{background:view==="dashboard"?C.gold:"transparent",color:view==="dashboard"?"#000":"#444",border:"none",borderRadius:6,padding:"5px 12px",fontWeight:700,fontSize:11,cursor:"pointer"}}>Dashboard</button>
              {students.map(s=><button key={s} onClick={()=>{setView("student");setActiveStudent(s);}} style={{background:activeStudent===s&&view==="student"?"#0f3460":"transparent",color:activeStudent===s&&view==="student"?C.gold:"#444",border:"none",borderRadius:6,padding:"5px 9px",fontWeight:700,fontSize:11,cursor:"pointer"}}>{s}</button>)}
            </>
          ):(
            <span style={{fontSize:12,color:C.gold,fontWeight:700}}>👤 {user}</span>
          )}
          <button onClick={()=>setUser(null)} style={{background:"#1a1a2e",color:"#444",border:"1px solid #1e1e2e",borderRadius:6,padding:"5px 10px",fontSize:10,cursor:"pointer",marginLeft:4}}>Sign Out</button>
        </div>
      </div>
      <div style={{maxWidth:1200,margin:"0 auto",padding:"24px 20px"}}>
        {isCoach
          ? view==="dashboard"
            ? <CoachDashboard allData={allData} students={students} onAddStudent={handleAddStudent} onRemoveStudent={handleRemoveStudent}/>
            : activeStudent&&<StudentTracker name={activeStudent} data={allData[activeStudent]||emptyStudent()} onSave={handleSave} isCoach/>
          : <StudentTracker name={user} data={allData[user]||emptyStudent()} onSave={handleSave} isCoach={false}/>
        }
      </div>
    </div>
  );
